  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ul className="divide-y rounded-lg border" aria-label="Clients">
        {initial.length === 0 ? (
          <li className="p-3 text-sm text-muted-foreground">No clients yet.</li>
        ) : null}
        {initial.map((c) => (
          <li key={c.id} className="p-3 text-sm">
            <span className="font-medium">{c.displayName}</span>
            {c.contactEmail ? <span className="text-muted-foreground"> · {c.contactEmail}</span> : null}
          </li>
        ))}
      </ul>
      <form onSubmit={onCreate} className="flex flex-wrap items-end gap-2" aria-label="Register client">
        <div className="flex flex-col gap-1">
          <label htmlFor="client-name" className="text-xs font-medium">Client name</label>
          <input id="client-name" className="rounded border px-2 py-1 text-sm" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        <div className="flex flex-col gap-1">
          <label htmlFor="client-email" className="text-xs font-medium">Contact email (optional)</label>
          <input id="client-email" type="email" className="rounded border px-2 py-1 text-sm" value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
        <Button type="submit" size="sm" disabled={pending}>Register client</Button>
      </form>
      {message ? (
